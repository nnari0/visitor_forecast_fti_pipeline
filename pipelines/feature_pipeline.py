...
Steps:
1. Read the raw data (CSV with visitor counts) and aggregate to an hourly grid
...
# ---------------------------------------------------------------------------
# 1) Load raw data + aggregate to an hourly grid
# ---------------------------------------------------------------------------
def load_visitor_data() -> pd.DataFrame:
    """Loads the CSV and aggregates the ~15-min samples to hourly means."""
    ...
    # Round down to full hours and average
    df["event_time"] = df["date"].dt.floor("h")
    hourly = (
        df.groupby("event_time", as_index=False)["visitors"]
        .mean()
        .rename(columns={"visitors": "visitors"})
    )
    hourly["visitors"] = hourly["visitors"].round().astype(int)

    print(f"  {len(hourly)} hourly values between "
          f"{hourly['event_time'].min()} and {hourly['event_time'].max()}")
    return hourly
...
def build_features(visitors: pd.DataFrame, weather: pd.DataFrame) -> pd.DataFrame:
    ...
    df = visitors.merge(weather, on="event_time", how="inner")

    # Expand to a complete hourly index so rolling windows compute consistently
    ...
    df = df.set_index("event_time").reindex(full_index).rename_axis("event_time").reset_index()
    ...
    df["visitors_avg_24h"] = (
        df["visitors"].shift(1).rolling(window=24, min_periods=6).mean()
    )
    df["visitors_avg_7d"] = (
        df["visitors"].shift(1).rolling(window=24 * 7, min_periods=24).mean()
    )

    # ---- Fill weather NaNs (short gaps in Open-Meteo) ----
